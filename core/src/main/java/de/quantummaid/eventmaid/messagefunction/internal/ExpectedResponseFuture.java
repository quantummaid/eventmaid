...
public final class ExpectedResponseFuture implements ResponseFuture {
    ...
    public synchronized void fullFill(final ProcessingContext<Object> response) {
        if (!isCancelled()) {
            ...
            finishFuture();
            final FollowUpAction followUpAction = followUpActionRef.get();
    ...
    private boolean hasExceptionInExecution() {
        return thrownExceptionRef.get() != null;
    }

    @Override
    public Object get() throws InterruptedException, ExecutionException {
        return getResponseRef(() -> responseRef.get().getPayload());
    }

    @Override
    public Object get(final long timeout, final TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
        return getResponseRef(() -> responseRef.get().getPayload(), timeout, unit);
    }

    @Override
    public Object getErrorResponse() throws InterruptedException, ExecutionException {
        return getResponseRef(() -> responseRef.get().getErrorPayload());
    }

    @Override
    public Object getErrorResponse(final long timeout,
                                   final TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
        return getResponseRef(() -> responseRef.get().getErrorPayload(), timeout, unit);
    }

    @Override
    public ProcessingContext<Object> getRaw() throws InterruptedException, ExecutionException {
        return getResponseRef(responseRef::get);
    }
    ...
    public ProcessingContext<Object> getRaw(
            final long timeout,
            final TimeUnit unit) throws InterruptedException, ExecutionException, TimeoutException {
        return getResponseRef(responseRef::get, timeout, unit);
    }

    private <T> T getResponseRef(final Supplier<T> responseSupplier) throws InterruptedException, ExecutionException {
        if (!isDone()) {
        ...
        if (hasExceptionInExecution()) {
            throw new ExecutionException(thrownExceptionRef.get());
        } else if (isCancelled()) {
        ...
        if (this.followUpActionRef.get() != null) {
            ...
            if (isDone()) {
                if (isCancelled()) {
                    ...
                    final ProcessingContext<Object> currentResponse = responseRef.get();
                    if (currentResponse != null) {
                        final Object payload = currentResponse.getPayload();
                        final Object errorPayload = currentResponse.getErrorPayload();
                        followUpAction.apply(payload, errorPayload, null);
                    } else {
